...
/* ── Custom branded field cursor ──────────────────────────────
   Matches the iconic Greenroots arrow pointer. The palette re-inks 
   itself to the theme under the pointer — paper sections wear 
   marigold × leaf with a paper stripe and ink outline; night sections 
   flip to leaf-bright × marigold with a cut-through ink stripe and 
   night-ink outline. The tip is pinned with zero lag and the body 
   swings with velocity tilt, swells over interactive elements, shrinks 
   to a quiet mono mark over form fields, and presses down on click.
   Touch devices automatically keep native OS behavior. ─────── */
...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    /* opt out only on touch-driven devices: multi-touch
       hardware whose primary pointer is coarse / hover-less. */
    if (
    ...
    const frame = (now: number) => {
      ...
      if (reduce.matches) {
        ...
        /* the tip is pinned to the pointer — zero lag */
        const vx = tx - prevTx;
        prevTx = tx;
      ...
      /* exact hotspot translation: tip sits at (HOT_X, HOT_Y) */
  ...
  return (
    ...
    >
      <div ref={glyphRef} className="cursor-glyph">
        ...
        >
          <defs>
            {/* bands run top-left → bottom-right:
                orange | white stripe | green | white stripe | orange */}
            <linearGradient
              id="gr-cursor-fill"
